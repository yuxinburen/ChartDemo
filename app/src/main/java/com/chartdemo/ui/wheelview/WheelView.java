...
package com.chartdemo.ui.wheelview;
...
public class WheelView extends View {
     ...
     * Shadow colors
     */
    private static final int[] SHADOWS_COLORS = { 0xCCFFFFFF, 0x77FFFFFF, 0x33FFFFFF };
	...
	private void initResourcesIfNecessary() {
        ...
        mTopShadow = new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, SHADOWS_COLORS); // 上半部分渐变颜色
        mBottomShadow = new GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP, SHADOWS_COLORS);// 下半部分渐变颜色
    }
	...
	private void drawItems(Canvas canvas) {
		...
		int mid = visibleItems / 2;
		for (int i = 0; i < count; i++) {
			child = itemsLayout.getChildAt(i);
			if(currentItem < visibleItems - mid) {
                if(i  == currentItem) {
					...
					if(child instanceof TextView) {
						((TextView)child).setTextColor(Color.parseColor("#9597ac"));
					}
				}
			} else {
				if (i == mid) {
	...
	private void drawCenterRect(Canvas canvas) {
        ...
        //上方的线
        mTopShadow.setBounds(0, 0, getWidth(), center - offset);
        mTopShadow.draw(canvas);

        //下方的线
        mBottomShadow.setBounds(0, center + offset, getWidth(), getHeight());
